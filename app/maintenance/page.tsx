...
export default async function MaintenancePage() {
  ...
  return (
    <main className="flex min-h-dvh items-center justify-center bg-ink-950 px-6 py-12 pt-safe lg:px-12">
      <MaintenanceWatcher offline />

      {/* Phones keep the stacked, centred block. From lg the page becomes a
        * two-column section: the illustration on one side, the words on the
        * other, both aligned to a shared baseline so the space is used rather
        * than left empty around a small centred card. */}
      <div className="w-full max-w-md text-center lg:grid lg:max-w-5xl lg:grid-cols-2 lg:items-center lg:gap-16 lg:text-left">
          <svg
            viewBox="0 0 200 120"
            role="img"
            aria-label="Servers under maintenance"
            className="mx-auto h-28 w-full max-w-[15rem] sm:h-36 sm:max-w-[18rem] lg:mx-0 lg:h-auto lg:w-full lg:max-w-none"
            fill="none"
            strokeLinecap="round"
        ...
        <div className="lg:min-w-0">
          <h1 className="mt-8 text-balance font-display text-2xl font-semibold tracking-tight text-neutral-100 sm:text-3xl lg:mt-0 lg:text-4xl">
            Ugnay is temporarily unavailable.
          </h1>

          {settings.siteMessage && (
            <p className="mx-auto mt-3 max-w-sm text-pretty text-sm leading-relaxed text-neutral-400 lg:mx-0 lg:mt-5 lg:max-w-md lg:text-base lg:leading-7">
              {settings.siteMessage}
            </p>
          )}

          {backAt && (
            <p className="mt-4 text-sm text-neutral-500 lg:mt-6 lg:inline-flex lg:items-center lg:gap-1.5 lg:rounded-xl lg:border lg:border-ink-800 lg:bg-ink-900 lg:px-3.5 lg:py-2">
              Expected back by{" "}
